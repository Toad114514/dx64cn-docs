# dxModDB 数据库
这里会收录一些部分 Coopdx 模组的简介，后面会慢慢补齐他们。
## 改版移植 / Romhack
  - Burtal Mario 64
  - Kaizo Mario 64
  - Return to Retroland
  - Super Mario 64: The Underworld
  - Super Mario 64 Land **[WIP]**
  - [Super Mario 74 (+EE)](./romhack/sm74.md)
  - Super Mario: The Last Impact **[WIP]**
  - Super Mario Treasure World
  - Star Revenge 5: Neo Blue Realm
  - [Star Road](./romhack/sr.md)
  - Star Road Autumn
  - Katze stuck in the toilet
## 游戏模式 / Game Mode
  - Arena
  - [Coopware (仿瓦力欧制造)](./gamemode/coopware.md)
  - Drench Game
  - Flood
  - Flood Expanded
  - Hide and Seek (DX)
  - MarioHunt (玛丽猎人)
## 动作集 / Movesets
  - [OMM Rebirth (奥德赛模组)](./movesets/omm.md)
  - mquake
## 角色选择模组 / Character Select
  - Baidi
  - Blue Archive CS Pack (蔚蓝档案)
  - Bowser (+Jr) Movesets
  - **Character Select v1.16.1 \[依赖库\]**
  - **Character Select v1.15.1 NameFix \[依赖库+名字修复\]**
  - DDLC CS Pack (心跳文学部)
  - Deltarune Ch4 Secret boss (+Movesets)
  - Extra Character Plus (扩展角色加强)
  - Paper Partners 64 + TTYD
  - Pizza Tower CS Pack (披萨塔)
  - Sonic Character: Rebooted
  - Speedrun Mario & Luigi
  - Super Mario World
  - Touhou CS Pack (东方Project)
  - Triple Baka Mod (三大傻术力口)
## 沙盒地图 / Hangout
  - Minecraft Hangout
## QoL
  - Weather Cycle
  - Day Night Cycle
  - Proximity Chat (仅电脑)
## 实用 / 辅助
  - Free Camera
  - Input Display
  - Level Display
  - Romhack Mods
## 小工具 / 其他
  - Minecraft+
  - Mari0
  - Guns Mods DX